...
static void fir_circular_init(struct fir_circular_t *fir, const float *coeff,
                              int fir_size, int buffer_size)
{
    fir->coeff = mem_alloc(L1_DM, fir_size * sizeof(float));
    fir->state = mem_alloc(L1_DM, fir_size * sizeof(float));
    fir->size = fir_size;
    fir->pos = 0;
...
static void fir_circular_init_dual_bank(struct fir_circular_t *fir,
